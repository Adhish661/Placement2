    ...
    const excelData = students.map(student => {
      ...
      if (profile?.skills) {
        ...
        if (Array.isArray(profile.skills.technical)) {
          skillStrs.push(...profile.skills.technical);
        }
      }
      const technicalSkillsStr = skillStrs.length ? skillStrs.join(', ') : '';
      const languagesStr = Array.isArray(profile?.skills?.languages) ? profile.skills.languages.join(', ') : '';

      return {
        'Name': student.name,
        'Email': student.email,
        'Full Name': profile?.basicDetails?.fullName || '',
        'Phone': profile?.basicDetails?.contactInfo?.phone || '',
        'Alternate Phone': profile?.basicDetails?.contactInfo?.alternatePhone || '',
        'Personal Email': profile?.basicDetails?.contactInfo?.personalEmail || '',
        'LinkedIn': profile?.basicDetails?.contactInfo?.linkedIn || '',
        'GitHub': profile?.basicDetails?.contactInfo?.github || '',
        ...
        'Technical Skills': technicalSkillsStr,
