...
import { sendDriveAssignment, sendDriveSharedToStudent } from '../utils/emailService.js';
...
export const shareDrive = asyncHandler(async (req, res) => {
  ...
  for (const student of studentsToShare) {
    try {
      if (student.email) {
        await sendDriveSharedToStudent(student.email, student.name, drive.companyName, drive.jobRole);
      }
    } catch (error) {
      console.error('Drive shared email error:', error);
    }
  }
